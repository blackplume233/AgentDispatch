---
description: "Migrated command: /trellis/qa-alpha"
---

# QA Alpha — 持久化 QA 调试环境

读取 `.opencode/skills/qa-engineer/SKILL.md` 理解 QA 测试方法论（黑盒为主、白盒为辅、智能判断、Issue 追踪），然后按照下方流程在**持久化环境**中执行 QA。
...
`/trellis/qa-alpha <mode> [args]`

| 模式 | 示例 | 行为 |
|------|------|------|
| **run** | `/trellis/qa-alpha run server-task-crud` | 在持久环境中执行场景 |
| **explore** | `/trellis/qa-alpha explore "并发创建 5 个任务"` | 在持久环境中即兴探索 |
| **status** | `/trellis/qa-alpha status` | 检查环境是否存活 |
| **restart** | `/trellis/qa-alpha restart` | 重建并重启环境（代码更新后） |
| **stop** | `/trellis/qa-alpha stop` | 手动停止环境（极少使用） |
| **create** | `/trellis/qa-alpha create "测试任务并发"` | 生成场景文件并在持久环境中执行 |
| **list** | `/trellis/qa-alpha list` | 列出所有场景 |
...
当用户执行 `/trellis/qa-alpha restart` 或 Phase 0.3 检测到代码更新时：
...
当用户执行 `/trellis/qa-alpha stop` 时：

1. 停止 Server + 杀死子进程树
2. 更新 env-state.json：`status → "stopped"`

**不删除任何文件**。下次 `/trellis/qa-alpha` 时会从 Phase 0 恢复。

---

## 与标准 QA 的对比

| 维度 | `/qa` (标准) | `/trellis/qa-alpha` (持久) |
|------|------------|------------------|
...
1. **不自动清理** — 这是 qa-alpha 的核心特性。环境在 `/trellis/qa-alpha stop` 后仍保留文件，只停止进程。
