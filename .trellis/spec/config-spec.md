...
interface ServerConfig {
  ...
  // [NEW 2026-03-02] 任务元数据 AI 增强预处理
  // 在 TaskService.createTask 前可选地调用 AI 优化 title/description/tags
  // 失败时 graceful fallback 到原始数据，不阻塞任务创建
  ai?: {
    enabled: boolean;              // 是否启用 AI 辅助，默认 false
    provider: 'acp' | 'openai' | 'custom';  // AI 提供方
    endpoint?: string;             // Provider endpoint（custom 模式必填）
    timeout: number;               // AI 处理超时（ms），默认 10000
    features: {
      titleOptimize: boolean;      // 自动优化模糊 title，默认 true
...
interface AgentConfig {
  ...
  // [CHANGED 2026-03-02] Worker 并发配置
  // 系统内部将 maxConcurrency > 1 的 Agent 展开为 N 个虚拟 Worker（worker-id:0, :1, ...）
  // 每个虚拟 Worker 复用现有单任务生命周期，共享同一 workDir，产物目录按任务隔离
  // ⚠️ 多进程共享 workDir 有文件冲突风险，涉及 git 操作等场景建议保持 1
  maxConcurrency?: number;         // 最大并发任务数，默认 1

  // [DEPRECATED 2026-03-02] 由 maxConcurrency 替代
  // allowMultiProcess: true 等价于 maxConcurrency: 2（保持向后兼容）
  // 新代码应使用 maxConcurrency
...
| 回调重试间隔 | `5s` |
| Worker 最大并发数 | `1` |
| 允许多进程 (DEPRECATED) | `false` |
| Server AI 辅助 | `false` (关闭) |
| Server AI 超时 | `10s` |
| 产物 zip 最大大小 | `500MB` |
| 产物上传时校验 | `true` |
| 产物保留天数 | `90` |
